...
class XAuto:
  ...
  def __init__(self, tmp_img='/dev/shm/tmp%d.png' % os.getpid(), default_threshold=0.7):
    self.d = X.Display()
  ...
  def find(self, img, threshold=None, rect=None, repeat=0, interval=0.2):
    if isinstance(img, str):
      img = Image(img)
    if rect is None:
      rect = (0, 0) + self.screensize
